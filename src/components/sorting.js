...
export function initSorting(columns) {
    return (query, state, action) => {
        ...
        if (action && action.name === 'sort'){
            ...
            // Берем текущий режим из атрибута кнопки (или ставим 'none' по умолчанию)
            const currentOrder = action.dataset.value || 'none';
            // Определяем следующий режим, используя карту переходов
            const nextOrder = sortMap[currentOrder];
            
            // Обновляем данные в кнопке, чтобы при следующем нажатии она знала новое состояние
            action.dataset.value = nextOrder;

            // Сохраняем значения для функции сортировки
            // Сохраним и применим как текущее следующее состояние из карты
